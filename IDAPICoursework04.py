...
#
def Mean(theData):
    return array([mean(col) for col in theData.astype(float).T])

def Covariance(theData):
    realData = theData.astype(float)
    N = realData.shape[0]
    U = realData - Mean(theData)
    return numpy.dot(U.T,U) / (N-1)
 
def CreateEigenfaceFiles(theBasis):
    for ef in range(theBasis.shape[0]):
        SaveEigenface(theBasis[ef],'./out/PrincipalComponent' + str(ef) + '.jpg')
 
def ProjectFace(theBasis, theMean, theFaceImage):
    W = theBasis.T
    X = array(theFaceImage) - array(theMean)
    return array(dot(X,W))
 
def CreatePartialReconstructions(aBasis, aMean, componentMags):
    rs = [aMean]
    V,L = aBasis,componentMags
    for i in range(len(componentMags)):
        rs += [dot(array(V[0:i]).T,array(L[0:i])) + array(aMean)]
    for r in range(len(rs)):
        SaveEigenface(rs[r],"./out/PartialReconstruction"+str(r)+".jpg")

def PrincipalComponents(theData):
    U = theData - Mean(theData)
    S = dot(U,U.T)
    L,V = linalg.eig(S) # L: eigvalues, V: eigvectors
    F = dot(U.T,V).T
    for i in range(len(L)):
        F[i] /= sqrt(abs(L[i]))
    ls = list(reversed([l[0] for l in sorted(enumerate(L), key=lambda x:x[1])]))
    Fsorted = array(F)
    for i in range(len(L)):
        Fsorted[i] = F[ls[i]]
    return Fsorted
...
AppendString("IDAPIResults04.txt","Coursework Four Results by cem13 and ik1410")
AppendString("IDAPIResults04.txt","")  

AppendString("IDAPIResults04.txt","Mean Vector for Hepatitis C")
AppendList("IDAPIResults04.txt",Mean(theData))
AppendString("IDAPIResults04.txt","")  

AppendString("IDAPIResults04.txt","Covariance matrix (Hepatitis C)")
S = Covariance(theData)
AppendArray("IDAPIResults04.txt",S)
AppendString("IDAPIResults04.txt","")  

AppendString("IDAPIResults04.txt","Magitutes for c.pgm (4.4)")
basis = ReadEigenfaceBasis()
m_img = array(ReadOneImage("MeanImage.jpg"))
magnitutes1 = ProjectFace(basis,m_img,ReadOneImage('c.pgm'))
AppendList("IDAPIResults04.txt",magnitutes1)

# Task 4.3 Created Faces
#CreateEigenfaceFiles(basis)

# Partial Recon of task 4.4
#CreatePartialReconstructions(basis, m_img, magnitutes1)

AppendString("IDAPIResults04.txt","Magitutes for c.pgm (4.4 NewBasis)")
basis = PrincipalComponents(array(ReadImages()))
m_img = Mean(array(ReadImages()))
magnitutes2 = ProjectFace(basis,m_img,ReadOneImage('c.pgm'))
AppendList("IDAPIResults04.txt",magnitutes2)
CreateEigenfaceFiles(basis)
CreatePartialReconstructions(basis, m_img, magnitutes2)
